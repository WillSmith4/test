...
auto_card_buy_menu() {
    while true; do
        choice=$(dialog --clear --title "Auto Card Buy" \
            --colors --begin 2 2 \
            --menu "\Z4Choose an option:\Zn" 15 50 4 \
            1 "\Z2Start Auto Buy\Zn" \
            2 "\Z3Show Best Upgrades\Zn" \
            3 "\Z5Back to Main Menu\Zn" \
            2>&1 >/dev/tty)

        case $choice in
            1) start_auto_buy ;;
            2) show_best_upgrades ;;
            3) return ;;
            *) display_error "Invalid option. Please try again." ;;
        esac
    done
...
start_auto_buy() {
    ...
    # Return to the auto card buy menu
    auto_card_buy_menu
}
